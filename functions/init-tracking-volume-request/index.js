...
const { SQS_VOLUME_TRACKING_REQUEST_QUEUE_URL, TOP_RANK } = process.env;
...
exports.handler = async (event) => {
    try {
        const {coinMap, coinList} = await getCoinInfo(TOP_RANK);
        const symbols = await getSymbols(coinList);

        console.log(`symbol top ${TOP_RANK}`, symbols, coinMap);
        ...
        for (const symbol of allValidSymbol) {
            ...
            const sendResult = await sendOrderToSqs(request, SQS_VOLUME_TRACKING_REQUEST_QUEUE_URL);
