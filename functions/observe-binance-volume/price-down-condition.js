...
const checkVolumeAndPriceDownCondition = async (symbol, alarmState, coinInfo, latestPrice) => {
    ...
    const part1m30 = chart30m.isAboveMA && latestPrice <= chart30m.lowestPriceInClosedCandle;
    const part2m30 = latestNotifyTelegramChart30m.currentCandleCloseTime < chart30m.closeTime;

    const part1H1 = chart1h.isAboveMA && latestPrice <= chart1h.lowestPriceInClosedCandle;
    const part2H1 = latestNotifyTelegramChart1h.currentCandleCloseTime < chart1h.closeTime;

    const part1H4 = chart4h.isAboveMA && latestPrice <= chart4h.lowestPriceInClosedCandle;
    ...
    console.log("SYMBOL PRICE DOWN", symbol, "part1m30", part1m30, "part2m30", part2m30, "isAlarmPrice30m", isAlarmPrice30m);
    console.log("SYMBOL PRICE DOWN", symbol, "part1H1", part1H1, "part2H1", part2H1, "isAlarmPrice1h", isAlarmPrice1h);
    console.log("SYMBOL PRICE DOWN", symbol, "part1H4", part1H1, "part2H1", part2H4, "isAlarmPrice4h", isAlarmPrice4h);
    ...
    if (isAlarmPrice30m || isAlarmPrice1h || isAlarmPrice4h) {
        const data = {
            ...
            chart30m: {
                ...
                isPriceBelowLowestPrice: latestPrice <= chart30m.lowestPriceInClosedCandle,
                isAlarm: chart30m.isAboveMA && latestPrice <= chart30m.lowestPriceInClosedCandle,
                lowestPriceInClosedCandle: chart30m.lowestPriceInClosedCandle,
            },
            chart1h: {
                volume: chart1h.volume,
                maVolume: chart1h.maVolume,
                isVolumeAboveMA: chart1h.isAboveMA,
                isPriceBelowLowestPrice: latestPrice <= chart1h.lowestPriceInClosedCandle,
                isAlarm: chart1h.isAboveMA && latestPrice <= chart1h.lowestPriceInClosedCandle,
                lowestPriceInClosedCandle: chart1h.lowestPriceInClosedCandle,
            },
            chart4h: {
                volume: chart4h.volume,
                maVolume: chart4h.maVolume,
                isVolumeAboveMA: chart4h.isAboveMA,
                isPriceBelowLowestPrice: latestPrice <= chart4h.lowestPriceInClosedCandle,
                isAlarm: chart4h.isAboveMA && latestPrice <= chart4h.lowestPriceInClosedCandle,
                lowestPriceInClosedCandle: chart4h.lowestPriceInClosedCandle,
            }
        }
...
const generateAlarmMessage = (symbol, rank, data) => {
    ...
    const chart4hMessage = `${chart4h.isAlarm ? "⚠️" : "🔷"} *4H* - *MA20*: ${chart4h.isAlarm ? "OVER SELL" : "STILL GOOD"}\n├ Lowest Price: ${chart4h.lowestPriceInClosedCandle}\n├ Price is ${chart4h.isPriceBelowLowestPrice ? "🔻" : "🔼"} lowest: ${getPercentDiff(latestPrice, chart4h.lowestPriceInClosedCandle)}%\n└ Volume ${chart4h.isVolumeAboveMA ? "🔺" : "🔽"} MA20: ${getPercentDiff(chart4h.volume, chart4h.maVolume)}%`;
    const chart1hMessage = `${chart1h.isAlarm ? "⚠️" : "🔷"} *1H* - *MA20*: ${chart1h.isAlarm ? "OVER SELL" : "STILL GOOD"}\n├ Lowest Price: ${chart1h.lowestPriceInClosedCandle}\n├ Price is ${chart1h.isPriceBelowLowestPrice ? "🔻" : "🔼"} lowest: ${getPercentDiff(latestPrice, chart1h.lowestPriceInClosedCandle)}%\n└ Volume ${chart1h.isVolumeAboveMA ? "🔺" : "🔽"} MA20: ${getPercentDiff(chart1h.volume, chart1h.maVolume)}%`;
    const chart30mMessage = `${chart30m.isAlarm ? "⚠️" : "🔷"} *30m* - *MA15*: ${chart30m.isAlarm ? "OVER SELL" : "STILL GOOD"}\n├ Lowest Price: ${chart30m.lowestPriceInClosedCandle}\n├ Price is ${chart30m.isPriceBelowLowestPrice ? "🔻" : "🔼"} lowest: ${getPercentDiff(latestPrice, chart30m.lowestPriceInClosedCandle)}%\n└ Volume ${chart30m.isVolumeAboveMA ? "🔺" : "🔽"} MA15: ${getPercentDiff(chart30m.volume, chart30m.maVolume)}%`;
